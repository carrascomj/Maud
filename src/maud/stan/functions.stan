functions {
  ...
  vector nn_predict(vector x, vector direction, matrix d_t_h, matrix[] h_t_h, matrix h_t_d,  matrix shared, row_vector[] hidden_bias, real y_bias) {
    int O = rows(h_t_d);
    int n_H = rows(d_t_h);
    int H = size(hidden_bias);
    vector[n_H] hidden_layers[H];
    vector[O] output_layer;
    int n_Edge = rows(shared);

    hidden_layers[1] = shared' * ((shared * inv_logit(d_t_h * x + hidden_bias[1]')) .* direction);
    for(h in 2:H) {
      hidden_layers[h] = shared' * ((shared * inv_logit(h_t_h[h-1] * hidden_layers[h-1] + hidden_bias[h]')) .* direction);
    }
